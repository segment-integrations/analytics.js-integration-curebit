...
var push = require('global-queue')('_talkableq');
...
Talkable.prototype.initialize = function() {
  var url = this.options.customUrl || '//d2jjzw81hqbuqv.cloudfront.net/integration/talkable-4.1.2.min.js';

  push('init', { site_id: this.options.siteId, server: this.options.server });

  this.load({ src: url }, this.ready);

  // throttle the call to `page` since talkable needs to append an iframe
 ...
 * campaign.
 *
 * http://www.talkable.com/docs/affiliate/registration
 *
 * This is throttled to prevent accidentally drawing the iframe multiple times,
 * from multiple `.page()` calls. The `250` is from the talkable script.
 *
 * @api private
 * @param {String} url
 * @param {String} id
 ...
 *
 * https://www.talkable.com/docs/ecommerce/custom
 *
 * @api public
 * @param {Track} track
 */

Talkable.prototype.orderCompleted = function(track) {
  var user = this.analytics.user();
